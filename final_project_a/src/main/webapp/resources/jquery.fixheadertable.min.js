(function($) {
	$.fn.fixheadertable = function(options) {
		...
		function buildHeaders(table) {
			_headers = $('<table class="head"/>').append(_colgroup).append(
					$('thead', table));
		...
		function adaptScroll(table) {
			_body = $('<div class="body ui-widget-content"></div>')
					.insertBefore(table).append(table);
		...
		return this
				.each(function() {
					_wrapper = $('<div/>').addClass(
							't_fixed_header ' + options.theme).insertBefore(
							this).append(this);
					buildTop(this);
					buildHeaders(this);
					adaptScroll(this);
					if (options.minWidth != null
							&& (parseInt(options.minWidth) != 'NaN')
							&& options.minWidth > 0) {
					...
					if (options.width != null
							&& (parseInt(options.width) != 'NaN')
							&& options.width > 0) {
						tampon.css('width', options.width + 'px');
					}
					var res = _wrapper.detach();
					var main_wrapper = $('<div class="t_fixed_header_main_wrapper ui-widget ui-widget-header '
							+ options.theme + '"></div>');
					...
					if (options.caption != '') {
						main_wrapper
								.prepend('<div class="t_fixed_header_caption ui-widget-header ui-corner-top">'
										+ options.caption + '</div>');
						main_wrapper.addClass('ui-corner-top');
					}
					var vertical_fake_scroll = $('<div style="right : 0px; width : 20px ; position : absolute; top : '
							+ (_headerscontainer.height()
									+ $('div.t_fixed_header_caption',
											main_wrapper).height() + 2)
							+ 'px; height : '
							+ height
							+ '; overflow-y : auto; overflow-x : hidden; z-index : 10000"><div class="" style="height : '
							+ $(this).height()
							+ 'px; width : '
							+ scrollWidth
					...
					if (options.whiteSpace == 'normal') {
						...
						vertical_fake_scroll.children().first().css("height",
								$(this).height());
					}
				});
	};
